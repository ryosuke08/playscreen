...
class test(Screen_abc):
    
    def display(self):
        super().update()
    
        #背景画像
        background = pygame.image.load("img/aa.jpg")
        SC.screen.blit(background,self.grid[0][0])
        #かぼちゃの画像
        panpkin = pygame.image.load("img\pipo-halloweenchara2016_01.png")
        SC.screen.blit(panpkin,(self.grid[18][10][0]+4,self.grid[18][10][1]))
        #player横の人間画像
        coinimg = pygame.image.load("img/sticon2b-3.png")
        Playerimg = pygame.image.load("img/ozaki32.png")
        #人間の画像
        character1 = pygame.image.load("img/pipo-charachip001a.png")
        SC.screen.blit(character1,self.grid[35][5])
        #お相撲さんの画像
        character2 = pygame.image.load("img/男_赤ふんどし.png")
        SC.screen.blit(character2,self.grid[35][9])
        #黒人の画像
        character3 = pygame.image.load("img/sample038.png")
        SC.screen.blit(character3,self.grid[35][13])

        #プレイヤー、順位、コイン数の縦位置の変数
        #プレイヤーらの画像のパスを保持している配列
        imgID = 6
        ranking = ["img/iti.png","img/nii.png","img/sani.png","img/yoni.png"]
        for i in ranking:
